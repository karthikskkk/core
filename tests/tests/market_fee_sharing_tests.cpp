...
struct reward_database_fixture : database_fixture
{
   ...
   void update_token( const account_id_type& issuer_id,
...
BOOST_AUTO_TEST_CASE(cannot_create_token_with_reward_percent_of_100_before_hf1774)
{
   try
   {
      ACTOR(issuer);
...
BOOST_AUTO_TEST_CASE(cannot_set_reward_percent_to_100_before_hf1774)
{
   try
   {
      ...
      GRAPHENE_CHECK_THROW(
                  update_token(issuer_id, issuer_private_key, usd_asset.get_id(), reward_percent, whitelist),
                  fc::assert_exception );
...
BOOST_AUTO_TEST_CASE(create_token_with_reward_percent_of_100_after_hf1774)
{
   try
   {
...
BOOST_AUTO_TEST_CASE(asset_rewards_test)
{
   try
   {
      ...
      transfer( dxpcore_account, alice.id, core_asset(1000000) );
      transfer( dxpcore_account, bob.id, core_asset(1000000) );
      transfer( dxpcore_account, izzy_id, core_asset(1000000) );
      transfer( dxpcore_account, jill_id, core_asset(1000000) );
      ...
      asset_id_type izzycoin_id = create_smarttoken( "IZZYCOIN", izzy_id, izzycoin_market_percent ).id;
      asset_id_type jillcoin_id = create_smarttoken( "JILLCOIN", jill_id, jillcoin_market_percent ).id;

      update_token(izzy_id, izzy_private_key, izzycoin_id, izzycoin_reward_percent);
      update_token(jill_id, jill_private_key, jillcoin_id, jillcoin_reward_percent);
...
BOOST_AUTO_TEST_CASE(asset_claim_reward_test)
{
   try
   {
      ...
      transfer( dxpcore_account, alice.get_id(), core_asset(1000) );
      transfer( dxpcore_account, bob.get_id(),   core_asset(1000) );
      transfer( dxpcore_account, izzy.get_id(),  core_asset(1000) );

      // update_token: set referrer percent
      update_token(jill_id, jill_private_key, jillcoin.get_id(), jillcoin_reward_percent);
...
BOOST_AUTO_TEST_CASE(create_actors)
{
   try
   {
      ...
      const account_object old   = create_account("old",   GRAPHENE_TEMP_ACCOUNT(db),
                                                           GRAPHENE_DXPCORE_ACCOUNT(db), 50u);
      ...
      transfer( dxpcore_account, alice.get_id(), core_asset(1000) );
      transfer( dxpcore_account, bob.get_id(),   core_asset(1000) );
      transfer( dxpcore_account, old.get_id(),   core_asset(1000) );
      transfer( dxpcore_account, tmp.get_id(),   core_asset(1000) );
      transfer( dxpcore_account, izzyregistrar.get_id(),  core_asset(1000) );
      transfer( dxpcore_account, izzyreferrer.get_id(),  core_asset(1000) );
      transfer( dxpcore_account, tempregistrar.get_id(),  core_asset(1000) );
   }
   FC_LOG_AND_RETHROW()
}

BOOST_AUTO_TEST_CASE(fee_shares_between_temp_acc_and_dxpcore_acc_before_hf_1800)
{
   try
   {
      ...
      update_token(jill_id, jill_private_key, jillcoin.get_id(), jillcoin_reward_percent, whitelist);

      BOOST_CHECK_EQUAL( get_market_fee_reward( GRAPHENE_TEMP_ACCOUNT(db), jillcoin), 0);
      BOOST_CHECK_EQUAL( get_market_fee_reward( GRAPHENE_DXPCORE_ACCOUNT(db), jillcoin), 0);
      ...
      BOOST_CHECK_GT( get_market_fee_reward( GRAPHENE_DXPCORE_ACCOUNT(db), jillcoin), 0);
...
BOOST_AUTO_TEST_CASE(fee_do_not_share_between_temp_acc_and_dxpcore_acc_after_hf_1800)
{
   try
   {
      INVOKE(create_actors);
...
BOOST_AUTO_TEST_CASE(create_token_via_proposal_test)
{
   try
   {
      ACTOR(issuer);
...
BOOST_AUTO_TEST_CASE(update_token_via_proposal_test)
{
   try
   {
      ACTOR(issuer);
...
BOOST_AUTO_TEST_CASE(issue_token){
   try
   {
...
BOOST_AUTO_TEST_CASE(accumulated_fees_before_hf_test)
{
   try
   {
      ...
      const asset_object &jillcoin = get_token("JILLCOIN");
      const asset_object &izzycoin = get_token("IZZYCOIN");
...
BOOST_AUTO_TEST_CASE(accumulated_fees_with_additional_options_after_hf_test)
{
   try
   {
      ...
      update_token(jill_id, jill_private_key, jillcoin.get_id(), reward_percent);
      update_token(izzy_id, izzy_private_key, izzycoin.get_id(), reward_percent);
...
BOOST_AUTO_TEST_CASE( create_vesting_balance_object_test )
{
   ...
   */
   try {
      ...
      create_vesting_balance_object(actor_id, vesting_balance_type::blockproducer);
      create_vesting_balance_object(actor_id, vesting_balance_type::blockproducer);

      create_vesting_balance_object(actor_id, vesting_balance_type::benefactor);
      create_vesting_balance_object(actor_id, vesting_balance_type::benefactor);
