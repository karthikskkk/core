This page shows how the dxpcore account can act using the proposed
transaction system. Specifically, as an example I'm using the
creation of Dxperts proposal 1.10.21, a proposal to update a
dxpcore-controlled Smarttoken to reduce `maximum_force_settlement_volume`
...
First check the asset to see what its current configuration is:
    ...
    {
      ...
      "smarttoken_data_id": "2.4.13"
    }

Then check its Smarttoken object to get the currently active options:
...
Then do `update_smarttoken` to update the options. Note we copy-paste
other fields from above; there is no way to selectively update only one field.

    >>> update_smarttoken "CNY" {"feed_lifetime_sec" : 86400, "minimum_feeds" : 7, "force_settlement_delay_sec" : 86400, "force_settlement_offset_percent" : 0, "maximum_force_settlement_volume" : 200, "short_backing_asset" : "1.3.0", "extensions" : []} false

If this was a privatized Smarttoken (i.e. a user-issued asset with feed),
you could simply set the `broadcast` parameter of the above command to
`true` and be done.

However this is a dxpcore-issued asset, so we have to use a proposed
...
Then set fees, sign and broadcast the transaction:
    ...
    >>> set_fees_on_builder_transaction $HANDLE DXP
...
- Remember to transfer enough to cover the fee to dxpcore account and set review period to at least `blockproducer_node`
