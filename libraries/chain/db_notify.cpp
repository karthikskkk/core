...
struct get_impacted_account_visitor
{
   ...
   void operator()( const asset_update_smarttoken_operation& op )
   {
      _impacted.insert( op.fee_payer() ); // issuer
   }
   ...
   void operator()( const blockproducer_create_operation& op )
   {
      _impacted.insert( op.fee_payer() ); // blockproducer_account
   }
   void operator()( const blockproducer_update_operation& op )
   {
      _impacted.insert( op.fee_payer() ); // blockproducer_account
   }
   ...
   void operator()( const dxpcore_member_create_operation& op )
   {
      _impacted.insert( op.fee_payer() ); // dxpcore_member_account
   }
   void operator()( const dxpcore_member_update_operation& op )
   {
      _impacted.insert( op.fee_payer() ); // dxpcore_member_account
   }
   void operator()( const dxpcore_member_update_global_parameters_operation& op )
   {
   ...
   void operator()( const benefactor_create_operation& op )
   {
      _impacted.insert( op.fee_payer() ); // owner
   }
   ...
   if( obj->id.space() == protocol_ids )
   {
      switch( (object_type)obj->id.type() )
      {
        ...
        case account_object_type:{
           ...
           break;
        } case dxpcore_member_object_type:{
           const auto& aobj = dynamic_cast<const dxpcore_member_object*>(obj);
           FC_ASSERT( aobj != nullptr );
           accounts.insert( aobj->dxpcore_member_account );
           break;
        } case blockproducer_object_type:{
           const auto& aobj = dynamic_cast<const blockproducer_object*>(obj);
           FC_ASSERT( aobj != nullptr );
           accounts.insert( aobj->blockproducer_account );
           break;
          ...
          break;
           ...
           accounts.insert( aobj->owner );
           break;
        } case benefactor_object_type:{
           const auto& aobj = dynamic_cast<const benefactor_object*>(obj);
           FC_ASSERT( aobj != nullptr );
           accounts.insert( aobj->benefactor_account );
           break;
   ...
   else if( obj->id.space() == implementation_ids )
   {
      switch( (impl_object_type)obj->id.type() )
      {
             ...
             case impl_asset_dynamic_data_object_type:
              break;
             case impl_asset_smarttoken_data_object_type:
              break;
             ...
             case impl_account_transaction_history_object_type: {
              ...
              break;
             case impl_blockproducer_schedule_object_type:
              break;
