...
def main():
    ...
    parser.add_argument("-n", "--num", metavar="N", default=11, type=int, help="number of init blockproducers")
    ...
    for i in range(opts.num):
        ...
        dxpcore.append({"owner_name" : "init"+str(i)})
    result = {
       "append" : {
       "initial_accounts" : wit_accounts },
       "replace" : {
       "initial_active_blockproducers" : opts.num,
       "initial_benefactor_candidates" : [],
       "initial_blockproducer_candidates" : wit_wits,
       "initial_dxpcore_candidates" : dxpcore,
        }
    }
